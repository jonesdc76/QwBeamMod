  ...
  for(Int_t j = 0; j < fNDetector; j++){
    ...
    for(Int_t k = 0; k < fNMonitor; k++){
      AMatrix(j,k) = (fChiSquareMinimization ? FindChiSquareMinAMatrix(j,k): 
		      AvDetectorSlope[fUseCoil[k]][j]);
      AMatrixE(j,k) = (fChiSquareMinimization ? FindChiSquareMinAMatrixError(j,k):
		       AvDetectorSlopeError[fUseCoil[k]][j]);
      if( (diagnostic.is_open() && diagnostic.good()) && run_avg ){
	diagnostic << AMatrix(j,k) << " +- " 
		   << AMatrixE(j,k) << " ";
      }
    }
    if( (diagnostic.is_open() && diagnostic.good()) && run_avg )
  ...
  for(Int_t imon = 0; imon < fNMonitor; imon++){
    for(Int_t icoil = 0; icoil < fNMonitor; icoil++){
      RMatrix(imon,icoil) = (fChiSquareMinimization ? 
			     ...
			     AvMonitorSlope[fUseCoil[icoil]][imon]);
      RMatrixE(imon,icoil) = (fChiSquareMinimization ? 
			     FindChiSquareMinRMatrixError(imon,icoil) :
			     AvMonitorSlopeError[fUseCoil[icoil]][imon]);

      if( (diagnostic.is_open() && diagnostic.good() && run_avg) ){
	diagnostic << RMatrix(imon,icoil) << " +- " 
		   << RMatrixE(imon,icoil) << " ";
      }
    }
    if( (diagnostic.is_open() && diagnostic.good()) && run_avg )
...
{
  if(!fChiSquareMinimization){
    for(int icoil=0;icoil<kNCoil;++icoil){
      int include_this = 1;
      for(int j=0;j<(int)fOmitCoil.size();++j){
	if(icoil == fOmitCoil[j]) include_this = 0;
      }
      if(include_this){
	fUseCoil.push_back(icoil);
      }
    }
    if((int)fUseCoil.size()!=fNMonitor){
      std::cout<<"Incorrect number of selected coils. Reverting to default coils ";
      for(int icoil=0;icoil<fNMonitor;++icoil){
	fUseCoil[icoil] = icoil;
	std::cout<< fUseCoil[icoil]<<" ";
      }
      std::cout<<std::endl;
    }else{
      std::cout<<"Using selected coils ";
      for(int icoil=0;icoil<fNMonitor;++icoil)
	std::cout<< fUseCoil[icoil]<<" ";
      std::cout<<std::endl;
    }
