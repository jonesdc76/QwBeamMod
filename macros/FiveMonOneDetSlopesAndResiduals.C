...
int FiveMonOneDetSlopesAndResiduals( int run, char *stem = ""){
  ...
  for(int icycle=0;icycle<nCycle;++icycle){
	...
	if(coil==0) {
	  cout<<"correction "<<dList[idet].Data()<<"_Coil"<<coil<<"  "<<vLEntry[icycle]<<" "<<vHEntry[icycle]<<endl;
	  cout<<"1 - (";
	}
	for(int imon=0;imon<nMON;++imon){
	  if(coil==0) 
	    printf("(%f-%f)*(%f)%s",mon[imon],vMonMean[imon][coil][icycle],
		   detSlopes[idet][imon][icycle],(imon==nMON-1 ? "" : " + "));
	  correction -= (mon[imon] - vMonMean[imon][coil][icycle]) 
	    * detSlopes[idet][imon][icycle] / val;
	}
	if(coil==0) 
	  cout<<"/("<<det[idet]<<"/"<<abs(vDetMean[idet][coil][icycle])<<")\n";
