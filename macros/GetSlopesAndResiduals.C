...
double GetAngle(double sine, double cosine, bool radians){

  //Because we are measuring everything relative to the drive frequency (ie.sine)
  //the angle is atan(cosine/sine) which is like measuring all angles relative
  //to the y-axis.
  double x = abs(sine);
  double y = abs(cosine);
  ...
  return (radians ? angle : angle * 180.0 / TMath::Pi());
}
...
int GetSlopesAndResiduals( int run, char *stem = "", bool fast = 0){

  //special case choice of phase offset to use for analysis
  bool sine_only = 1;
  bool cosine_only = 0;
  TString nm = stem;
  if(nm.Contains("cosine_only")){
    sine_only = 0;
    cosine_only = 1;
  }else if(nm.Contains("sine_only")){
    sine_only = 1;
    cosine_only = 0;
  }
  if(sine_only) cosine_only = 0;
  if(cosine_only) sine_only = 0;
  ...
  /////////////////////////////////////////////////////////
  int nDet;
  TString MonitorList[nMOD], DetectorList[nDET], mList[nMOD], dList[nDET];
  ...
  chSlopes->Add(Form("%s/../MacrocycleSlopesTree_ChiSqMin.set0.root", 
		     gSystem->Getenv("MPS_ONLY_ROOTFILES")));

//   TChain *chSlopes2 = new TChain("slopes");
//   chSlopes->Add(Form("%s/../MacrocycleSlopesTree_ChiSqMin.set0_monitor_test.root",
// 		     gSystem->Getenv("MPS_ONLY_ROOTFILES")));
//   chSlopes->AddFriend(chSlopes2);
  ...
  for(int i=n;i<chSlopes->GetEntries();++i){
	...
	double val = chSlopes->GetLeaf(nm.Data())->GetValue();
	vMonCoeff[imon][icoil].push_back(val);
	if(icoil<nMOD){
	  ...
	  if(nm.Contains("Slope"))
	    ...
	    mean *= 1.0e-3;
	  else if(nm.Contains("md"))
	    mean *= 1.0e-6;
	  vMonMean[imon][icoil].push_back(mean);
	}
	printf("%s_Coil%i %f   %f\n", mList[imon].Data(),icoil, 
	       vMonCoeff[imon][icoil].back(), vMonMean[imon][icoil%nMON].back());
      }
  ...
  //Fill matrix and invert to find new slopes
  /////////////////////////////////////////////////////////
  ...
  for(int icycle=0;icycle<nCycle;++icycle){
    double det_angle[nMOD], offset[nMOD]={0,0,0,0,0};
    for(int imod=0;imod<nMOD;++imod){
      cout<<"Angle:" <<GetAngle(vDetCoeff[0][imod][icycle],vDetCoeff[0][imod+nMOD][icycle],1)<<endl;
      det_angle[imod] =  GetAngle(vDetCoeff[0][imod][icycle],
    				  vDetCoeff[0][imod+nMOD][icycle],1);
      if(sine_only)
	offset[imod] = det_angle[imod];
      else if(cosine_only)
	offset[imod] = det_angle[imod]-TMath::Pi()/2.0;
    }
    cout<<endl;
    ...
    for(int irow=0;irow<nMON;++irow){
      for(int icol=0;icol<nMON;++icol){
	double phi = det_angle[irow] - offset[irow];
	phi=0;
	double sum = vMonCoeff[icol][irow][icycle]* cos(phi);
	sum += vMonCoeff[icol][irow+nMOD][icycle] * sin(phi);
	//	cout<<irow<<" "<<icol<<endl;
	R(irow, icol) = sum;
      }
    }
    ...
    for(int idet=0;idet<nDet;++idet){
      TVectorD A(nMON);
      for(int irow=0;irow<nMON;++irow){
	double val =  sqrt(pow(vDetCoeff[idet][irow][icycle],2) + 
			   pow(vDetCoeff[idet][irow+nMOD][icycle],2));
	val *= cos(offset[irow]);
	A(irow) = val;
      }
      A.Print();
  ...
  /////////////////////////////////////////////////////////
	...
	for(int idet=0;idet<nDet;++idet){
      ...
      for(int idet=0;idet<nDet;++idet){
	for(int imod=0;imod<nMOD;++imod){
	  TGraph gr = TGraph((int)v_ramp_filled[imod].size(), &v_ramp_filled[imod][0], 
