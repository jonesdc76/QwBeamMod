...
int GetSlopesAndResidualsFast( int run, char *stem = "",
			       int coil_to_tweak = 0, 
  ...
  if(tweak_factor!=1.0){
    TString nm = stem;
    if(!nm.Contains(Form("%0.1f",abs(tweak_factor)))){
  ...
  chSlopes->Add(Form("%s/../MacrocycleSlopesTree_ChiSqMin.set0_tgt.root", 
		     gSystem->Getenv("MPS_ONLY_ROOTFILES")));
  cout<<Form("%s/../MacrocycleSlopesTree_ChiSqMin.set0.root", 
	     gSystem->Getenv("MPS_ONLY_ROOTFILES"))<<endl;  
  cout<<chSlopes->GetEntries()<<" entries.\n";;
...
//   chSlopes->AddFriend(chSlopes2);
  ...
  /////////////////////////////////////////////////////////
  double tweak[nCOIL] = {-0.35,-3.24,-0.78,-0.3,2.65,(-3.09+0.95)/2, (4.04-1.13)/2, (-1.77-1.91)/2,(3.71+3.35)/2,(-0.5-3.3)/2.0};
  int nCycle = (int)vId.size();
  cout<<nCycle<<" macrocycles found.\n";
  for(int idet = 0;idet<nDet;++idet)
    for(int icoil = 0;icoil<nCOIL;++icoil)
    for(int icycle = 0;icycle<nCycle;++icycle)
      vDetCoeff[idet][icoil][icycle] -= tweak[icoil];
  ...
  for(int icycle=0;icycle<nCycle;++icycle){
	...
	if(nMON<nCOIL){
	  for(int icoil=0;icoil<nCOIL;++icoil){
	    val += vMonCoeff[irow][icoil][icycle]*vMonCoeff[icol][icoil][icycle];
	  }
	}else{
	  val = vMonCoeff[icol][irow][icycle];
	}
	R(irow, icol) = val;
      }
    }
    ...
    R.Print();
    TVectorD REigenvalues(nMON);
    TMatrixD REigenvectors(nMON, nMON);
    REigenvectors = R.EigenVectors(REigenvalues);
	...
	}else val =  vDetCoeff[idet][irow][icycle];
	A(irow) = val;
      }
	...
	detSlopes[idet][irow][icycle] = slope[irow] * unit_conversion;
      ...
      cout<<DetectorList[idet].Data()<<" vector:"<<endl;
      A.Print();
    }
  }
  /////////////////////////////////////////////////////////
...
// 	      abs(vDetMean[idet][coil][icycle])<<")\n";
  ...
  cosineData.open(Form("%s%s/macrocycle_slopes/run%iCosineAmpl%s.set0.dat",
		     gSystem->Getenv("BMOD_OUT"), stem, run, chisquare),fstream::out);
  if(sineData.is_open() && sineData.good() && cosineData.is_open() 
     && cosineData.good()){
