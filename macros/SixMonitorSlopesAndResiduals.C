...
const int nMON = 6, nCOIL = 10, nMOD = 5, nDET = 100;
...
int SixMonitorSlopesAndResiduals( int run, char *stem = "", bool fast = 0){
...
//   chSlopes->AddFriend(chSlopes2);
  ...
  //Tweak coefficients and invert to find new slopes
  /////////////////////////////////////////////////////////
  ...
  coeffFile.open(Form("%s%s/macrocycle_slopes/"
		      "coil_coeffs_%i_ChiSqMin.set0.dat", 
		      gSystem->Getenv("BMOD_OUT"), stem, run), fstream::out);
  
  if(coeffFile.is_open() && coeffFile.good()){
    for(int icycle=0;icycle<nCycle;++icycle){
      coeffFile<<"Macrocycle "<<vMacrocycle[icycle]<<" \t";
      for(Int_t imod = 0;imod<nMOD;imod++){
	coeffFile<<"Mod"<<imod<<" "<<vCyclet[imod][icycle]<<" \t";
      }
      coeffFile<<"Id "<<run<<vMacrocycle[icycle]<<"  first_entry "<<
	vLEntry[icycle]<<"  last_entry "<<vHEntry[icycle]<<std::endl;
      for(int imon=0;imon<nMON;++imon){
	coeffFile<<Form("mon %s\n",MonitorList[imon].Data());
	for(int icoil=0;icoil<nCOIL;++icoil){
	  double mean_conv = (MonitorList[imon].Contains("Slope")? 1.0e4 :1.0);
	  double coef_conv = (MonitorList[imon].Contains("Slope")? 0.01 :0.001);
	  coeffFile<<Form("%+14.7e\t%10.3e    %+14.7e\t%10.3e\n",
			  vMonCoeff[imon][icoil][icycle]*coef_conv, 0.0,
			  vMonMean[imon][icoil%nMOD][icycle]*mean_conv, 0.0);
	}
      }
      for(int idet=0;idet<nDet;++idet){
	coeffFile<<Form("det %s\n",DetectorList[idet].Data());
	for(int icoil=0;icoil<nCOIL;++icoil){
	  coeffFile<<Form("%+14.7e \t%10.3e    %+14.7e \t%10.3e\n",
			  vDetCoeff[idet][icoil][icycle]*1.0e-6, 0.0,
			  vDetMean[idet][icoil%nMOD][icycle], 0.0);
	}
      }
    }
  }else{
    std::cout<<"Coefficients file not open.Exiting.\n";
    return -1;
  }
  coeffFile.close();
  /////////////////////////////////////////////////////////
