...
Int_t MakeRegressedDBrootfile(Int_t run_period = 1, TString set = "std", 
			      bool pass5c =0, char *dir ="/net/data2/paschkelab1")
{
  Bool_t debug = 0;
  TString listsDir = "/home/ek4px/lists/detectorLists/";
  TString reducedFileDir = Form("%s/%sreduced_slugfiles/", dir,
				(pass5c ? "pass5c_" : ""));
...
//   ///////////////////////////
  TFile *newfile = new TFile(Form("%s/run%i/hydrogen_cell_%s_regressed_tree.root",
				  gSystem->Getenv("DB_ROOTFILES"), run_period, set.Data()),
			     "recreate");
  if(newfile==0){
