...
int GetSlopesAndResiduals_beta( int run, char *stem = ""){
...
//   chSlopes->AddFriend(chSlopes2);
  ...
  //calculate new residuals using matrices.
  /////////////////////////////////////////////////////////
  int d = 0;
  for(int icycle=0;icycle<nCycle;++icycle){
    TVectorD v(nMON), DetCoeff(nCOIL);
    for(int imon=0;imon<nMON;++imon){
      v(imon) = detSlopes[d][imon][icycle];
      DetCoeff(imon) = vDetCoeff[d][imon][icycle];
      DetCoeff(imon+nMON) = vDetCoeff[d][imon+nMON][icycle];
    }
	...
	val += MonCoeffMatrix(icoil,imon) * detSlopes[d][imon][icycle]*unit_conversion ;
      }
      v2(icoil) = val;
    }
    DetCoeff -= v2;
  ...
  //Find new residuals by fitting
  /////////////////////////////////////////////////////////
  TF1 *f = new TF1("f", "[0]+[1]*sin(x*TMath::Pi()/180.0)+[2]*cos(x*TMath::Pi()/180.0)",
		   lBound, hBound);
  ...
  vector<vector<vector<double> > >v_cos_res, v_sin_res, v_cos_res_err, v_sin_res_err;
