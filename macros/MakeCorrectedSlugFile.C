...
Int_t MakeCorrectedSlugFile(int slug, TString stem ="",
			    TString indisk = "/net/data2/paschkelab2",
			    TString outdisk = "/net/data2/paschkelab2")
{
  bool pass5c = false;
  TString name = "";
  if(stem.Contains("pass5c")){
    pass5c = true;
    name = "pass5c_";
  }
  if(pass5c)cout<<"Using pass5c slopesfile format\n";
  ...
  TString rootFileDir = Form("%s/%sreduced_slugfiles/run%i/", indisk.Data(), 
			     name.Data(), run_period);
  TString outFileDir = Form("%s/%sreduced_slugfiles/run%i/", outdisk.Data(),
			    name.Data(), run_period);
  ...
  if(pass5c){
    ifstream slopesFile(Form("%sslopeList_%s_slug%i.dat", slopeListsDir.Data(), 
			   detector[0].Data(), slug));
    slopes_exist = (slopesFile.is_open() && slopesFile.good());
    cout<<Form("%sslopeList_%s_slug%i.dat\n", slopeListsDir.Data(), 
	       detector[0].Data(), slug);
  }else{
  ...
  if(!slopes_exist){
    ...
    //    cout<<Form("%sslopeList_%s_slug%i.dat", slopeListsDir.Data(), 
    //	       detector[0].Data(), slug)<<endl;
    return -1;
  }
  ...
  for (int iDet=0; iDet<nDet; iDet++) {
    ...
    if(pass5c)
      fileName = Form("slopeList_%s_slug%i.dat", detector[iDet].Data(), slug);
