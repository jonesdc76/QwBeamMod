...
{
  Bool_t debug = 0;
  TString correctedFileDir = Form("/net/data3/paschkelab4/%sreduced_slugfiles/",
				(pass5c ? "pass5c_" : ""));
  ...
  TString good_slugs_filename = Form("%s%s/slopelists/good_slugs.dat", 
  ...
  if(pass5c){
    ifstream good_slugs_file(good_slugs_filename.Data());
    if( !(good_slugs_file.is_open() && good_slugs_file.good()) ){
      cout<<"Good slugs file not found. Run the following command from the "
	  <<"slopelists  directory then rerun program:\n";
      cout<<"ls slopeList_asym_qwk_mdallbars_slug* |"
	  <<" cut -d'.' -f1 | cut -d'g' -f2 > good_slugs.dat\n";
      return -1;
    }
    int sl;
    while(!good_slugs_file.eof()){
      good_slugs_file>>sl;
      goodSlugs[sl] = 1;
      good_slugs_file.peek();
    }
    good_slugs_file.close();
...
//     //check if slopes are expected to exist
    ...
    if(pass5c){
      slopesShouldExist = goodSlugs[slug_number];
    }else{
